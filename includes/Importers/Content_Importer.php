...
use COSS\Importers\Helpers\Helper;
use COSS\Importers\Helpers\Importer_Alterator;
use COSS\Logger;
use COSS\Importers\WP\WP_Import;
use WP_Error;
use WP_REST_Request;
...
class Content_Importer {
	...
	public function import_remote_xml( WP_REST_Request $request ) {
		if ( ! current_user_can( 'manage_options' ) ) {
			...
			return new WP_REST_Response(
				array(
					'data'    => 'codeless__ob_permission_err_1',
					'success' => false,
				)
			);
		}

		do_action( 'codeless_ob_before_xml_import' );
		...
		if ( empty( $content_file_url ) ) {
			...
			return new WP_REST_Response(
				array(
					'data'    => 'codeless__ob_remote_err_1',
					'success' => false,
				)
			);
		}
		...
		if ( ! isset( $body['source'] ) || empty( $body['source'] ) ) {
			...
			return new WP_REST_Response(
				array(
					'data'    => 'codeless__ob_remote_err_2',
					'success' => false,
				)
			);
		}
		...
		if ( $body['source'] === 'remote' ) {
			...
			$response_file = wp_remote_get( add_query_arg( 'key', apply_filters( 'codeless_product_specular_license_key', 'free' ), $content_file_url ), $request_args );
		...
		do_action( 'codeless_ob_after_xml_import' );
		...
		do_action( 'codeless_ob_after_front_page_setup' );

		// Set shop pages.
		if ( isset( $body['shopPages'] ) ) {
			$this->setup_shop_pages( $body['shopPages'], $body['demoSlug'] );
		}
		do_action( 'codeless_ob_after_shop_pages_setup' );
	...
	public function save_xhr_return_path( $content ) {
		...
		$file_path     = $wp_upload_dir['basedir'] . '/codeless-demo-import.xml';
		require_once( ABSPATH . '/wp-admin/includes/file.php' );
	...
	public function import_file( $file_path, $req_body = array(), $builder = '' ) {
		if ( empty( $file_path ) || ! file_exists( $file_path ) || ! is_readable( $file_path ) ) {
			return new WP_Error( 'codeless__ob_content_err_1', 'No content file' );
		}
