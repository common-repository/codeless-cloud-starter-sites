...
namespace COSS\Importers;

use Plugin_Upgrader;
use COSS\Importers\Helpers\Quiet_Skin;
use COSS\Importers\Helpers\Quiet_Skin_Legacy;
use COSS\Logger;
use WP_REST_Request;
...
class Plugin_Importer {
	...
	private $exception_mapping = array(
		...
		'cowidgets-elementor-addons'       => 'cowidgets.php',
		...
		if ( ! current_user_can( 'install_plugins' ) ) {
			...
			return new WP_REST_Response(
				array(
					'success' => false,
					'log'     => $this->log,
					'data'    => 'codeless__ob_perm_err_1',
				)
			);
		}

		do_action( 'codeless_ob_before_plugins_install' );
	...
	public function run_plugins_install( $plugins_array ) {
		...
		do_action( 'codeless_ob_after_plugins_install' );

		update_option( 'codeless_ob_plugins_installed', 'yes' );
	}

	private function installExternalPlugin( $url ){
	...
	private function remove_possible_redirects() {
		...
		update_option( 'codeless_blocks_settings_redirect', false );
	}
	...
	private function install_single_plugin( $plugin_slug ) {
		...
		do_action( 'codeless_ob_before_single_plugin_install', $plugin_slug );
		...
		if( $plugin_slug == 'js_composer' )
			$url = 'https://codeless.co/specular/plugins/js_composer.zip';
		if( $plugin_slug == 'revslider' )
			$url = 'https://codeless.co/specular/plugins/revslider.zip';
		if( $plugin_slug == 'specular-wpbakery-addons' )
			$url = 'https://codeless.co/specular/plugins/specular-wpbakery-addons.zip';
		...
		do_action( 'codeless_ob_after_single_plugin_install', $plugin_slug );
	}
	...
	private function activate_single_plugin( $plugin_slug ) {
		...
		do_action( 'codeless_ob_before_single_plugin_activation', $plugin_slug );
		...
		do_action( 'codeless_ob_after_single_plugin_activation', $plugin_slug );
	}
