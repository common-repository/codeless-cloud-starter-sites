...
class Theme_Mods_Importer {
	...
	public function import_theme_mods( WP_REST_Request $request ) {
		if ( ! current_user_can( 'customize' ) ) {
			return new WP_REST_Response(
				array(
					'data'    => 'codeless__ob_permission_err_2',
					'success' => false,
				)
			);
		}

		do_action( 'codeless_ob_before_customizer_import' );
		...
		if ( ! isset( $data['source_url'] ) || empty( $data['source_url'] ) ) {
			...
			return new WP_REST_Response(
				array(
					'data'    => 'codeless__ob_theme_mods_err_1',
					'success' => false,
				)
			);
		}
		...
		if ( ! isset( $data['theme_mods'] ) || empty( $data['theme_mods'] ) ) {
			return new WP_REST_Response(
				array(
					'data'    => 'codeless__ob_theme_mods_err_2',
					'success' => false,
				)
			);
		}
		...
		foreach ( $this->theme_mods as $mod => $value ) {
			if ( $mod === '__codeless_import_menus_location' ) {
				continue;
			}
			if ( $value === 'true' ) {
				$value = true;
		...
		if ( isset( $this->theme_mods['__coss_import_menus_location'] ) ) {
			$menus = $this->theme_mods['__coss_import_menus_location'];
			$this->setup_nav_menus( $menus );
		}

		do_action( 'codeless_ob_after_customizer_import' );
	...
	public function setup_nav_menus( $menus ) {
		do_action( 'codeless_ob_before_nav_menus_setup' );
		...
		do_action( 'codeless_ob_after_nav_menus_setup' );
	}
	...
	private function change_theme_mods_root_url( &$item ) {
		do_action( 'codeless_ob_before_change_theme_mods_root_url' );
		$item = $this->replace_image_urls( $item );
		do_action( 'codeless_ob_after_change_theme_mods_root_url' );
	}
