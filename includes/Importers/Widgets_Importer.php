...
class Widgets_Importer {
	...
	public function import_widgets( WP_REST_Request $request ) {
		...
		do_action( 'codeless_ob_before_widgets_import' );
		...
		if ( is_wp_error( $import ) ) {
			return new WP_REST_Response(
				array(
					'data'    => 'codeless__ob_widgets_err_1',
					'success' => false,
				)
			);
		}

		do_action( 'codeless_ob_after_widgets_import' );
	...
	public function actually_import( $data ) {
		...
		if ( empty( $data ) || ! is_array( $data ) ) {
			return new WP_Error( 'codeless__ob_widget_err_1' );
		}
		...
		foreach ( $data as $sidebar_id => $widgets ) {
			...
			foreach ( $widgets as $widget_instance_id => $widget ) {
				...
				if ( ! $fail ) {
					...
					do_action( 'codeless_ob_after_single_widget_import', $after_widget_import );
