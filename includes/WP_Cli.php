 ...
 * WP CLI commands.
 *
 * Author:          Andrei Baicus <andrei@codeless.co>
...
use COSS\Importers\Content_Importer;
use COSS\Importers\Helpers\Helper;
use COSS\Importers\Plugin_Importer;
use COSS\Importers\Theme_Mods_Importer;
use COSS\Importers\Widgets_Importer;
...
class WP_Cli {
	 ...
	 */
	const CLI_NAMESPACE = 'codeless-si';
	/**
	 * List of commands.
	 *
	 ...
	 * ## EXAMPLES
	 *
	 *     wp codeless-si import codeless-restaurant
	 *
	...
	private function import_theme_mods( $json ) {
		...
		if ( isset( $json['theme_mods'] ) && ! empty( $json['theme_mods'] ) ) {
			...
			foreach ( $json['theme_mods'] as $key => $value ) {
				if ( $key === '__codeless_import_menus_location' ) {
					continue;
				}
				if ( $value === 'true' ) {
					$value = true;
		...
		if ( isset( $json['theme_mods']['__codeless_import_menus_location'] ) ) {
			$this->theme_mods_importer->setup_nav_menus( $json['theme_mods']['__codeless_import_menus_location'] );
		}
...
$codeless_ob_cli = new WP_Cli();
$codeless_ob_cli->load_commands();
